    ...
    stages {
        ...
        stage('Building') {
            steps {
                ...
                sh  "${SCRIPT_DIR}/scripts/shared/build-generic/13_build-lpunpack.sh \"${BUILD_TARGET}\""
                sh  "${SCRIPT_DIR}/scripts/shared/build-generic/14_lpunpack-super-imgs.sh \"${BUILD_NUMBER}\" \"${BUILD_TARGET}\""
            }
        }
        stage('Analysis') {
            steps {
